...
* **Maximizing Player: Computer (O)**
    * Tries to achieve the **highest score** (a win).
* **Minimizing Player: Human (X)**
    * Tries to force the **lowest score** (a loss for the AI).
...
1.  **Clone the repository:** Copy the source code to your machine:
    ```bash
...
4.  **Run the game:** Execute the main Python script from your terminal
    ```bash
